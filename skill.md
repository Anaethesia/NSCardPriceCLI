...
1. **输出格式**：所有命令的正式输出是 JSON，写到 **stdout**；日志与错误信息写到 **stderr**。解析数据只看 stdout。
2. **退出码**：`0` = 成功（含 unavailable/skipped 状态）；`1` = 有商家返回 error；`2` = 用法错误/未找到。判断失败必须以退出码为准，`非 0 即失败`。
3. **商家指定**：`-m/--merchant` 可多次使用，接受 key 或中文名（如 `-m buerjia` 或 `-m 不二家`）；不传 = 查询全部 4 家。
4. **游戏匹配**：`query` 参数按中文名/关键词模糊匹配（大小写不敏感），常用简称与别名自动识别；精确 slug 匹配暂未启用。
...
| `nscardprice query --mul 词1 词2 词3` | 多关键词同时查询（空格分隔，最多 5 个，结果按关键词顺序去重合并） |
...
# -> 也按关键词处理，3 条全部命中（精确 slug 匹配暂未启用）
```

**教训**：中文关键词常返回多个条目。当用户问"旷野之息多少钱"而未说明版本时，应**展示所有命中条目并让用户确认**；需要唯一结果就用精确中文名，或把这些卡带维护进 `custom.json` 再 `query -c` 查。
